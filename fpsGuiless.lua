...
getgenv().globalPart = nil  -- For R15 Games: {UpperTorso, LowerTorso, HumanoidRootPart, Head} | For R6 Games: {Head, Torso, HumanoidRootPart}  
    ...
    getgenv().GetObscuringObjects = function(T)
        if T and T:FindFirstChild(getgenv().globalPart) and Client and Client.Character:FindFirstChild("Head") then 
            local RayPos = workspace:FindPartOnRay(RNew(
                T[getgenv().globalPart].Position, Client.Character.Head.Position)
            )
    ...
    Mouse.KeyDown:Connect(function(a)
        if not (Uis:GetFocusedTextBox()) then 
            if a == Uis:GetStringForKeyCode(getgenv().main.Key) and AimlockTarget == nil then
                pcall(function()
            ...
            elseif a == Uis:GetStringForKeyCode(getgenv().main.Key) and AimlockTarget ~= nil then
    ...
    RService.RenderStepped:Connect(function()
       ...
       if AimlockTarget.Character.HuDDDDDDDDDDWmanoid.FloorMaterial == Enum.Material.Air then
         ...
         getgenv().globalPart = getgenv().OldAimPart
